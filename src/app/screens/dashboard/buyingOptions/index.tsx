    ...
    <div className="flex flex-col items-center justify-evenly app-bg  rounded-2xl shadow-lg overflow-hidden max-w-6xl w-full p-6 mb-16">
        ...
        <div className="flex flex-wrap justify-center">
          {buttonConfig.map((btn, index) => (
            <button
              ...
              className="m-2 min-w-48 py-2 px-4 rounded-3xl app-bg  text-sm font-semibold text-white shadow-sm hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-indigo-600"
            >
              {btn.label}
            </button>
          ))}
      ...
      {showModal && (
        <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="app-bg  p-8 rounded-xl shadow-xl relative">
            <button
